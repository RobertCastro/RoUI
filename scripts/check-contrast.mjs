...
function collect(node, path = [], inherited = {}) {
  ...
  if (Object.hasOwn(node, "$value")) {
    tokens.set(path.join("."), {
      id: path.join("."),
      value: node.$value,
      cssVariable: metadata.cssVariable ?? `--ro-${path.at(-1)}`,
    });
    return;
  }
...
const baseVariables = new Map(
  [...tokens.values()]
    .filter((token) => !token.id.startsWith("themes."))
    .map((token) => [token.cssVariable, token]),
);
const aliasId = (value) =>
...
function resolveValue(value, overrides, stack = []) {
  ...
  if (stack.includes(token.id))
    throw new Error(`Alias circular: ${[...stack, token.id].join(" -> ")}`);
  const override = overrides.get(token.cssVariable);
  const inheritedAlias = override === `{${token.id}}`;
  return resolveValue(inheritedAlias ? token.value : (override ?? token.value), overrides, [
    ...stack,
    token.id,
  ]);
}
...
function luminance(hex) {
  ...
  if (!/^([0-9a-f]{3}|[0-9a-f]{6})$/i.test(raw))
    throw new Error(`Color no hexagonal para contraste: ${hex}`);
  const expanded =
    raw.length === 3
      ? raw
          .split("")
      ...
      : raw;
  return [0, 2, 4]
    .map((index) => Number.parseInt(expanded.slice(index, index + 2), 16) / 255)
...
const pairs = [
  ["page", "--ro-bg", "--ro-text"],
  ["surface", "--ro-surface", "--ro-text"],
  ["button-primary", "--ro-button-primary-bg", "--ro-button-primary-fg"],
  ["button-dark", "--ro-button-dark-bg", "--ro-button-dark-fg"],
  ["control", "--ro-control-bg", "--ro-control-fg"],
];
let failures = 0;
for (const theme of ["light", "dark", "high-contrast"]) {
  const overrides = new Map(
    [...tokens.values()]
      .filter((token) => token.id.startsWith(`themes.${theme}.`))
  ...
  for (const [name, backgroundVariable, foregroundVariable] of pairs) {
    ...
    if (value < 4.5) {
      console.error(`Contraste insuficiente para ${theme}/${name}: ${background} y ${foreground}`);
      failures += 1;
    }
  }
}
